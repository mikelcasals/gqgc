...
parser.add_argument('--valid_size', type=float, default=0.1, help="Fraction of data to use for validation")
...
def main():
    ...
    train_X, train_y, valid_X, valid_y, test_X, test_y = split_data(X, y, args.train_size, args.valid_size, args.test_size, args.seed)

    scaler = choose_norm(args.norm_name)
    train_X, valid_X, test_X = normalize_features(scaler, train_X, valid_X, test_X)

    #transform to graph format
    full_graph_data_path = args.outdir + "/graphdata_" + str(args.num_samples)
    if args.part_dist:
...
def load_and_subsample_data(num_samples=10000, part_dist = True, filter_outliers=False, subsampled_data_path = 'aux_data/subsampled_data'):
