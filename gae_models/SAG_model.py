...
class SAG_model(GAE):
    ...
    def __init__(self, device="cpu", hpars={}):
        ...
        self.hp_SAG_model.update((k, hpars[k]) for k in self.hp_SAG_model.keys() & hpars.keys())
        self.hp_gae.update((k, self.hp_SAG_model[k]) for k in self.hp_SAG_model.keys())
        
        self.gae_type = self.hp_SAG_model["gae_type"]
    ...
    def instantiate_encoder(self):
        ...
        conv = SAGEConv(self.num_node_features, self.shapes[0])
        self.down_list.append(conv)
        for i in range(self.depth - 1):
            pool = SAGPooling(self.shapes[i], self.c_rate[i])
            self.pool_list.append(pool)
            conv = SAGEConv(self.shapes[i], self.shapes[i + 1])
            self.down_list.append(conv)
        pool = SAGPooling(self.shapes[-1], self.c_rate[-1])
        self.pool_list.append(pool)
