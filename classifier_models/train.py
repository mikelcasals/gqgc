...
from .quantum.data import QuantumGraphsLoader

def main(args):
    gae_util.set_seeds(args["seed"])
    device = gae_util.define_torch_device(args["device"])
    outdir = "./trained_classifiers/" + args["outdir"] + "/"
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    
    if args["compressed"]:
        print("Compressing data...")
        util.save_compressed_data(device, args, "train")
        util.save_compressed_data(device, args, "valid")
        train_graphs = gae_data.SelectGraph(args['compressed_data_path']+"/train")
        valid_graphs = gae_data.SelectGraph(args['compressed_data_path']+"/valid")
    else:
    ...
    #if args["quantum"]:
    #    train_loader = QuantumGraphsLoader(train_loader)
    #    valid_loader = QuantumGraphsLoader(valid_loader)
    
    #Model definition
    model = util.choose_classifier_model(args["classifier_type"], device, args)
