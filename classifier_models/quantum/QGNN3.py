...
class QGNN3(Classifier):
    def __init__(self, device="cpu", hpars={}):
        ...
        self.num_qubits_per_node = (self.num_features+1)//2

        self.alphas = torch.nn.Parameter(0.01*torch.randn(self.num_layers, self.num_features), requires_grad=True)
        self.betas = torch.nn.Parameter(0.01*torch.randn(self.num_layers, self.num_qubits_per_node), requires_grad=True)
    ...
    def quantum_circuit(self, x, edge_index, edge_weight, alphas, betas):
        ...
        """
        vf.equivariant_ansatz_v2(x,edge_index, edge_weight, alphas, betas)
