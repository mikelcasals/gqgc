...
class SAG_model(nn.Module):

    def __init__(self, device="cpu", hpars={}):

        super().__init__()
        self.hp = {
            "ae_type": "vanilla_SAG_model",
            "input_size": 13,
            "kernels": 1,
            "depth":3,
            "rate":0.80,
        ...
        #Encoder
        conv = SAGEConv(self.input_size, self.shapes[0])
        self.down_list.append(conv)
        for i in range(self.depth - 1):
            pool = SAGPooling(self.shapes[i], self.rate[i])
            self.pool_list.append(pool)
            conv = SAGEConv(self.shapes[i], self.shapes[i + 1])
            self.down_list.append(conv)
        pool = SAGPooling(self.shapes[-1], self.rate[-1])
        self.pool_list.append(pool)
