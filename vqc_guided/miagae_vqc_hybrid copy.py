...
from autoencoders.MIAGAE_classifier import MIAGAE_classifier
...
from .util import convert_to_adj_matrix, transform_data_for_classifier
...
class MIAGAE_VQC(MIAGAE_classifier):
    """
    ...
    def __init__(self, qdevice, device, hpars):
        ...
        #self.circuit = qml.qnode(self.qdevice, diff_method = self.diff_method, interface="torch")(self.qcircuit_prueba)
    ...
    def qcircuit_prueba(self, inputs, betas, gammas):
        qml.Hadamard(0)
        qml.Hadamard(1)

        qml.RX(inputs*betas[0], wires=0)
        qml.RX(inputs*gammas[0], wires=1)
    ...
    def compute_loss_acc_rocauc(self, data, betas=None, gammas=None):
        ...
        #acc = self.compute_accuracy(class_output, data.y.cpu())
        #rocauc = self.compute_roc_auc(class_output, data.y.cpu())
        acc = 0
        rocauc = 0
